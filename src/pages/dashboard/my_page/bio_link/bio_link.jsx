...
const Bio_link = () => {
  ...
  } = useGetPageIdQuery({ id: pageId }, { skip });
...
const Form = (props) => {
  ...
  const onSubmit = (data) => {
    ...
    let cardDataToSend = {
      full_name: data.name,
      email: data.email,
      mobile: data.mobilePrefix.value + "-" + data.mobile,
      phone: data.landlinePrefix.value + "-" + data.phone,
  ...
  useEffect(() => {
    ...
    if (props.userData?.personal_information[0]?.home_phone) {
      COUNTRIES.map(
        (item) =>
          item.prefix ===
